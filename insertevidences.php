...
if (isset($_POST["insert"])) {
    ...
    if (isset($_FILES["image"]["tmp_name"]) && !empty($_FILES["image"]["tmp_name"])) {
        ...
        // Check if file2 was uploaded successfully
        if (isset($_FILES["image2"]["tmp_name"]) && !empty($_FILES["image2"]["tmp_name"])) {
            ...
            // Handle file2 upload error
            $message = "Fingerprint file not uploaded!";
            echo "<script>if(confirm('$message')){document.location.href='insertevidences.php'};</script>";
        }
    } else {
        // Handle file upload error
        $message = "Photo file not uploaded!";
        echo "<script>if(confirm('$message')){document.location.href='insertevidences.php'};</script>";
    }
}
